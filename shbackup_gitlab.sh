...
Backup () {
  ...
  last_filename=$(ls -Atrs --block-size=M ${MNT_DST}*.tar | tail -n1)
  msg="$(echo "Gitlab Backup Passed successfully with the size of ${last_filename}")"
